...
class SellerRazorPayPayment extends CI_Controller
{
    public $created_at;
    public $apikey;
    ...
    function makepayment($latestID,$id,$amount,$currency){
        ...
        var options = {
            ...
            "handler": function (response){
                ...
                location.replace("'.base_url().'seller-rzp-fetch-response/"+response.razorpay_payment_id+"/"+response.razorpay_order_id+"/"+response.razorpay_signature+"/'.$latestID.'")
    ...
    function fetchResponse($payment_id,$order_id,$signature,$wallet_id){
        ...
        redirect('seller-travpay-add-money');
    }
    ...
    function makepaymentRemit($latestID,$id,$amount,$currency){
        ...
        var options = {
            ...
            "handler": function (response){
                ...
                alert(response.razorpay_order_id);
                alert(response.razorpay_signature);
                location.replace("'.base_url().'SellerRazorPayPayment/fetchResponseRemit/"+response.razorpay_payment_id+"/"+response.razorpay_order_id+"/"+response.razorpay_signature+"/'.$latestID.'")
    ...
    function fetchResponseRemit($payment_id,$order_id,$signature,$wallet_id){
        $userinfo = $this->session->userdata('seller');
        $bdata = $this->session->userdata('bdata');
        ...
        if($this->admin->insert_data('tbl_wallet_details',$data)){
            ...
            if($walletID){
                ...
                if($this->admin->insert_data('tbl_wallet_transaction_details',$wddata)){
                    ...
                    $this->failed("Error");
                    redirect('Seller/remit_type/uto');
                }
